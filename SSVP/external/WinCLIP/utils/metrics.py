...
def metric_cal(scores, gt_list, gt_mask_list, cal_pro=False):
    ...
    # compute average precision (AP) for image and pixel levels
    try:
        img_ap = average_precision_score(gt_list, img_scores) * 100
    except Exception:
        img_ap = 0

    try:
        p_ap = average_precision_score(gt_mask.flatten(), scores.flatten()) * 100
    except Exception:
        p_ap = 0

    result_dict = {'i_roc': img_roc_auc * 100, 'i_ap': img_ap, 'p_roc': per_pixel_rocauc * 100, 'p_ap': p_ap,
     'p_pro': pro_auc_score * 100, 'i_f1': img_f1 * 100, 'p_f1': pxl_f1 * 100, 'r_f1': max_f1_region * 100}
