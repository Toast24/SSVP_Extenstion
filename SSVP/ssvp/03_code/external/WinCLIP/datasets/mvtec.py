...
# Resolve MVTec datasets directory relative to repository root (workspace)
# This works regardless of current working directory when scripts are launched
REPO_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', '..'))
MVTEC2D_DIR = os.path.join(REPO_ROOT, '04_data', 'datasets')
...
def load_mvtec(category, k_shot, experiment_indx):
    ...
    # Support datasets where the category folder may be nested (e.g., transistor/transistor/*).
    cat_base = os.path.join(MVTEC2D_DIR, category)
    if os.path.isdir(os.path.join(cat_base, category)):
        cat_dir = os.path.join(cat_base, category)
    else:
        cat_dir = cat_base

    test_img_path = os.path.join(cat_dir, 'test')
    train_img_path = os.path.join(cat_dir, 'train')
    ground_truth_path = os.path.join(cat_dir, 'ground_truth')
