...
class WinClipAD(torch.nn.Module):
    def __init__(self, out_size_h, out_size_w, device, backbone, pretrained_dataset, scales, precision='fp32', **kwargs):
        ...
        self.device = device
        self.get_model(backbone, pretrained_dataset, scales, img_resize=kwargs.get('img_resize', None))
        self.phrase_form = '{}'
    ...
    def get_model(self, backbone, pretrained_dataset, scales, img_resize=None):

        assert backbone in valid_backbones
        assert pretrained_dataset in valid_pretrained_datasets

        # ensure the visual tower is created with the same image size as the preprocessing
        model, _, _ = CLIPAD.create_model_and_transforms(
            model_name=backbone,
            pretrained=pretrained_dataset,
            scales=scales,
            precision=self.precision,
            force_image_size=img_resize if img_resize is not None else None,
        )
